...
public abstract class ModelDragon extends ModelBase {
     ...
     * @param maxFlapAngle the max angle from 0 (positive and negative) the wing can deviate from
     
    protected void flapWings(EntityLiving entity, int tick, final int flapSeconds, int maxFlapAngle) {
        int flapTicks = flapSeconds*20;
        boolean moveWingsDown = tick%flapTicks==0 && tick%flapTicks>=flapTicks/2;
        
        wing.rotateAngleZ += (moveWingsDown?-1:1)*(Math.toRadians(maxFlapAngle)/flapTicks);
    }*/
