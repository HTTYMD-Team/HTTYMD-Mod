...
public class ItemUtils {
	...
	public static Set<Block> getEffectiveForToolType(EnumToolType type) {
		...
		switch (type) {
		case PICKAXE:
			result = Sets.newHashSet(new Block[] { Blocks.cobblestone, Blocks.double_stone_slab, Blocks.stone_slab,
			...
			break;
		case AXE:
			result = Sets.newHashSet(new Block[] { Blocks.planks, Blocks.bookshelf, Blocks.log, Blocks.log2,
					Blocks.chest, Blocks.pumpkin, Blocks.lit_pumpkin });
			break;
		case SHOVEL:
			result = Sets.newHashSet(new Block[] { Blocks.grass, Blocks.dirt, Blocks.sand, Blocks.gravel,
